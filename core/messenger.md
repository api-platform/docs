...
Since Symfony 4.3, the messenger is using the native serializer (`Symfony\Component\Messenger\Transport\Serialization\PhpSerializer`). If you're using Doctrine entities as the Message, note that relation fields will not be in sync with Doctrine's Object Manager and you will have to merge those by hand in the message handler.
...
Note that when doing so it'll have a negative impact on performances. Also, if API Platform's serializer is used, it will work only for resources tagged as `ApiResource` and not for DTOs. Indeed, our normalizer expects resources to have an IRI, in other words at least one `GET` operation declared in symfony's `RouteCollection` (see also [](./operations.md)).
