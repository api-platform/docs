...
API Platform Core allows to register custom operations for collections and items.
...
API Platform allows registering custom operations for collections and items.
Custom operations allow to customize routing information (like the URL and the HTTP method),
the controller to use (default to the built-in actions applicable for the given HTTP method) and a context that will be
passed to documentation generators.
