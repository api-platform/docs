...
As stated in [the general design considerations](../usage-and-configuration/design.md), 
in most cases [the DTO pattern](https://en.wikipedia.org/wiki/Data_transfer_object) should be implemented using an API Resource class 
representing the public data model exposed through the API and [a custom data provider](../fetching-and-persisting-data/data-providers.md). 
In such cases, the class marked with `@ApiResource` will act as a DTO.
...
The `input` attribute is used during [the deserialization process](../serialization/index.md), when transforming the user-provided data to a resource instance.
Similarly, the `output` attribute is used during [the serialization process](../serialization/index.md). This class represents how the `Book` resource will be represented in the `Response`.
...
![Diagram post input output](images/api-platform-post-i-o.png)
