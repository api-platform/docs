...
[For most use cases, better extensions points, working both with REST and GraphQL, are available](../getting-started/extending.md).

API Platform Core implements the [Action-Domain-Responder](https://github.com/pmjones/adr) pattern. This implementation
is covered in depth in the [Creating custom operations and controllers](../usage-and-configuration/operations.md)
chapter.
...
------------------------------|--------------------|---------------------------------------------|----------|-------------
`AddFormatListener`           | `kernel.request`   | None                                        | 7        | Guesses the best response format ([content negotiation](../getting-started/content-negotiation.md))
`ReadListener`                | `kernel.request`   | `PRE_READ`, `POST_READ`                     | 4        | Retrieves data from the persistence system using the [data providers](../fetching-and-persisting-data/data-providers.md) (`GET`, `PUT`, `DELETE`)
...
`DenyAccessListener`          | `kernel.request`   | None                                        | 1        | Enforces [access control](../security/security.md) using Security expressions
`ValidateListener`            | `kernel.view`      | `PRE_VALIDATE`, `POST_VALIDATE`             | 64       | [Validates data](validation.md) (`POST`, `PUT`)
`WriteListener`               | `kernel.view`      | `PRE_WRITE`, `POST_WRITE`                   | 32       | Persists changes in the persistence system using the [data persisters](../fetching-and-persisting-data/data-persisters.md) (`POST`, `PUT`, `DELETE`)
`SerializeListener`           | `kernel.view`      | `PRE_SERIALIZE`, `POST_SERIALIZE`           | 16       | Serializes the PHP entity in string [according to the request format](../getting-started/content-negotiation.md)
...
attribute of an operation](operations.md)):
