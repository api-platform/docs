...
just a data structure with no or minimal behaviors) and will be automatically converted to [Hydra](../documenting-specifying-your-api/extending-jsonld-context.md),
[OpenAPI](../documenting-specifying-your-api/swagger.md) and [GraphQL](../graphql/index.md) documentations or schemas by API Platform (there is a 1-1 mapping
...
the [`DataProviderInterface`](../fetching-and-persisting-data/data-providers.md). Basically, the data provider will query the persistence system (RDBMS,
...
data provided by API Platform's resource object [hydrated by the serializer](../serialization/index.md).
To do so, there is another interface to implement: [`DataPersisterInterface`](../fetching-and-persisting-data/data-persisters.md).
...
or [CQRS](https://martinfowler.com/bliki/CQRS.html) thanks to [the Messenger Component integration](../fetching-and-persisting-data/messenger.md) and the [DTO support](../serialization/dto.md).

Last but not least, to create [Event Sourcing](https://martinfowler.com/eaaDev/EventSourcing.html)-based systems, a convenient
approach is:

* to persist data in an event store using a Messenger handler or a custom [data persister](../fetching-and-persisting-data/data-persisters.md)
...
You can then benefit from the built-in Doctrine filters, sorting, pagination, auto-joins and all of [the extension points](../getting-started/extending.md) provided by API Platform.
