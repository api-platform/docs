...
This guide assumes you installed API-Platform with the [Distribution](installation.md#using-the-api-platform-distribution).
...
HTTP methods. Don't forget to [disable the ones you don't want](../usage-and-configuration/operations.md#enabling-and-disabling-operations)!

Try the `GET` operation on the collection. The book we added appears. When the collection contains more than 30 items,
the pagination will automatically show up, [and this is entirely configurable](../pagination-filters-sorting/pagination.md). You may be interested
in [adding some filters and adding sorts to the collection](../pagination-filters-sorting/index.md) as well.
...
It is useful for open data, SEO and interoperability, especially when [used with open vocabularies such as Schema.org](https://blog.schema.org/2013/06/schemaorg-and-json-ld.html)
...
However, API Platform natively [supports many other formats](content-negotiation.md) including [GraphQL](https://graphql.org/)
(we'll get to it), [JSON API](https://jsonapi.org/), [HAL](http://stateless.co/hal_specification.html), raw [JSON](https://www.json.org/),
...
You can also easily [add support for other formats](content-negotiation.md) and it's up to you to choose which format
...
By the way, you may want to [embed documents](../serialization/index.md) instead of referencing them
(e.g. to reduce the number of HTTP requests). You can even [let the client select only the properties it needs](../pagination-filters-sorting/property.md).
...
[filters](../pagination-filters-sorting/index.md) and [access control rules](../security/security.md).
...
![The React Progressive Web App](../../distribution/images/api-platform-2.5-pwa-react.png)
...
Now that you learned the basics, be sure to read [the general design considerations](../usage-and-configuration/design.md) and [how to extend API Platform](extending.md) to understand how API Platform is designed, and how to hook your custom business logic!
...
Then, there are many more features to learn! Read [the full documentation](../../distribution/index.md) to discover how to use them
...
designed to create complex API-driven projects, far beyond simple CRUD apps. It benefits from [**strong extension points**](extending.md)
and it is **continuously optimized for [performance](../caching-performance-optimization/cache.md).** It powers numerous high traffic websites.
...
[API Platform Core Library: Enabling the Built-in HTTP Cache Invalidation System](../caching-performance-optimization/cache.md#enabling-the-built-in-http-cache-invalidation-system).
